...
async function getActiveProducts() {
    try {
        ...
        return {
            products: ((data ?? []) as ProductListItemRow[]).map((product) => ({
                id: product.id,
                name: product.name,
                slug: product.slug,
                ...
                primaryImage: getPrimaryImage(product.product_images),
            })),
            hasError: false,
        };
    } catch (error) {
                ...
                <div className="max-w-2xl space-y-5">
                    ...
                    <div className="space-y-4">
                        ...
                        <p className="text-base leading-8 text-stone-400">
                            A quiet edit of fragrance and lifestyle essentials,
                            drawn from the current Sombre catalog.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                        {products.map((product) => (
                            <ProductCard
                                ...
                                imageAlt={
                                    product.primaryImage?.alt_text ?? null
                                }
                            />
                        ))}
                    </div>
