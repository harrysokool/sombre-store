...
import AdminOrdersPage, { metadata } from "./page";
...
describe("admin orders list page", () => {
  ...
  it("sets the Orders route title while inheriting dashboard noindex metadata", () => {
    expect(metadata).toEqual({ title: "Orders" });
  });

  it("checks admin authentication before reading any orders", async () => {
    mocks.requireAdminUser.mockRejectedValue(
      new Error("redirect to admin login"),
    );

    await expect(AdminOrdersPage()).rejects.toThrow("redirect to admin login");
    expect(mocks.listAdminOrders).not.toHaveBeenCalled();
  });

  it("renders the shared Orders page header even when there are no orders", async () => {
